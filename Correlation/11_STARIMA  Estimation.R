...
p_order <- 4          # AR order (try: 1, 2, 3, 4) - SAME AS UNIFORM/DISTANCE
...
if (!is.matrix(C) || nrow(C) != ncol(C)) halt("Correlation weight matrix must be square")

# lag-0 = I, lag-1 = C, lag-k = row-normalized C^k
wlist_correlation <- vector("list", max_spatial_lag + 1L)
wlist_correlation[[1]] <- diag(nrow(C))
wlist_correlation[[2]] <- C
...
# CORRELATION WEIGHTS: Fully dynamic spatial lags activation
