# ============================================================================
# RESIDUAL DIAGNOSTICS - correlation WEIGHTS
# ============================================================================
...
if (!file.exists("output/11_starima_correlation.RData")) {
  stop("❌ Missing required file: output/11_starima_ucorrelationRData")
}
load("output/11_starima_correlation.RData")

if (!file.exists("output/07_spatial_weights_correlation_only.RData")) {
  stop("❌ Missing required file: output/07_spatial_weights_ucorrelation_onlyRData")
}
load("output/07_spatial_weights_correlation_only.RData")

# Extract data from correlation_results
residuals_matrix <- correlation_results$residuals
residual_stats <- correlation_results$residual_stats
p_order <- correlation_results$orders$p
q_order <- correlation_results$orders$q
d_order <- correlation_results$orders$d
max_spatial_lag <- correlation_results$orders$max_spatial_lag
...
wlist_correlation <- list(diag(nrow(W)), W)

# Normalize weights
for (i in 1:length(wlist_correlation)) {
  for (j in 1:nrow(wlist_correlation[[i]])) {
    rs <- sum(wlist_correlation[[i]][j, ])
    if (rs > 0) {
      wlist_correlation[[i]][j, ] <- wlist_correlation[[i]][j, ] / rs
    } else if (i == 1) {
      wlist_correlation[[i]][j, j] <- 1
    }
  }
}
...
ggsave("plots/12_correlation_residual_acf.png", acf_plot, width = 10, height = 6, dpi = 300)
ggsave("plots/12_correlation_residual_pacf.png", pacf_plot, width = 10, height = 6, dpi = 300)
ggsave("plots/12_correlation_qq_plot.png", qq_plot, width = 8, height = 6, dpi = 300)
...
save(diagnostic_results, residual_acf, residual_pacf, 
     file = "output/12_diagnostic_correlation.RData")

cat("\n✅ Diagnostic completed and saved to: output/12_diagnostic_correlation.RData\n")
cat("✅ Plots saved to: plots/12_correlation_*.png\n")