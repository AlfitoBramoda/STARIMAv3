...
# Purpose: Estimate STARIMA(p,d,q) model using distance spatial weights
...
if (!exists("spatial_weights") || is.null(spatial_weights$distance_idw)) halt("'spatial_weights$distance_idw' not found")
...
U <- spatial_weights$distance_idw
...
ar_mask <- if (p_order > 0) matrix(FALSE, p_order, max_spatial_lag + 1) else matrix(FALSE, 0, max_spatial_lag + 1)
ma_mask <- if (q_order > 0) matrix(FALSE, q_order, max_spatial_lag + 1) else matrix(FALSE, 0, max_spatial_lag + 1)

# Activate all temporal lags for spatial lag 0 (within-region effects)
if (p_order > 0) {
  for (p in 1:p_order) ar_mask[p, 1] <- TRUE
}
if (q_order > 0) {
  for (q in 1:q_order) ma_mask[q, 1] <- TRUE
}
