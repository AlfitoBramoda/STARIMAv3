...
load("output/06_distance_matrix_km.RData")
cat("📊 Data loaded: rainfall_matrix (", nrow(rainfall_matrix), "x", ncol(rainfall_matrix), ")\n")
cat("📏 Distance matrix loaded from 06_distance_matrix_km.RData\n")
...
# ============================================================================
cat("\n1️⃣ Using Pre-calculated Distance Matrix (km)...\n")

# Use the distance matrix from 06_Ubah KM.R
...
cat("Distance range: [", round(min(distance_matrix[distance_matrix > 0]), 3), 
    ",", round(max(distance_matrix), 3), "] km\n")
...
save(spatial_weights, coordinates, distance_weights, distance_matrix, 
     ...
     file = "output/07_spatial_weights_idw.RData")
