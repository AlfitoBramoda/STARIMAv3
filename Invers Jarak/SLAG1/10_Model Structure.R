# ============================================================================
# STARMA Forecasting Pipeline - Phase 3: STARIMA Estimation (distance Final)
# File: 10_Model_Structure_distance_Final.R
# Purpose: Create AR/MA mask matrices for distance weight type + plotting ACF-style
...
cat("=== STARMA MODEL STRUCTURE DEFINITION (distance WEIGHTS) ===\n\n")
...
create_mask_plot <- function(mask_matrix, title) {
  if (is.null(mask_matrix) || length(mask_matrix) == 0 ||
    ...
    cat("⚠ Warning: Empty mask matrix for", title, "\n")
  ...
  if (length(param_vector) != nrow(mask_df)) {
    cat("⚠ Warning: Parameter vector length mismatch in", title, "\n")
...
tryCatch({
  ...
  cat("⚠ Warning:", paste0(weight_type, "_ar"), "not found, using default\n")
})

tryCatch({
  ma_obj <- get(paste0(weight_type, "_ma"), envir = .GlobalEnv)
}, error = function(e) {
  cat("⚠ Warning:", paste0(weight_type, "_ma"), "not found, using default\n")
})

# Load identification results for correct orders
if (file.exists("output/09_stpacf_distance_only.RData")) {
  load("output/09_stpacf_distance_only.RData")
  p_order <- distance_ar$suggested_p      # from STPACF
  q_order <- distance_ma$suggested_q      # from STACF
} else {
  p_order <- 3; q_order <- 3  # Use STARIMA(3,1,3) as default
}

# FIXED: Calculate parameters with parsimonious approach
...
# SAVE RESULTS
# ============================================================================
save(model_structures, plots, file = "output/10_model_structure_distance_weights.RData")

cat("\n✅ STARIMA model structure for distance weights saved to 'output/10_model_structure_distance_weights.RData'\n")
cat("✅ Integration orders (d, D) added to model\n")
cat("🎯 Ready for STARIMA estimation for distance weight type\n")