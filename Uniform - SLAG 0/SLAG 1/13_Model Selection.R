...
# Purpose: Evaluate and finalize STARIMA model using uniform spatial weights (SLAG 1)
...
cat("=== STARIMA MODEL SELECTION (uniform ONLY - SLAG 1) ===\n\n")
cat(sprintf("📋 Evaluating: %s - uniform Weights (SLAG 1)\n\n", model_name))

# ============================================================================
# MODEL FIT STATISTICS
# ============================================================================
cat("📊 Model Fit Statistics (uniform Weights - SLAG 1):\n")
cat("==========================================\n")

uniform_stats <- uniform_results_slag1$fit_statistics

model_summary <- data.frame(
  Model = model_name,
  Spatial_Weight_Type = "uniform-Based (Cross-correlation)",
...
# ============================================================================
cat("\n🔍 Parameter Significance (uniform Weights - SLAG 1):\n")
cat("============================================\n")

uniform_coef <- uniform_results_slag1$coefficients
...
# ============================================================================
cat("\n🔬 Diagnostic Summary (uniform Weights - SLAG 1):\n")
cat("========================================\n")
...
# ============================================================================
cat("\n🧮 Parameter Consistency (uniform Only - SLAG 1):\n")
cat("========================================\n")
...
cat(sprintf("📊 Final Model Selected: %s — uniform-Based Spatial Weights (SLAG 1)\n", model_name))
...
fit_plot <- ggplot(model_summary, aes(x = Model, y = AIC, fill = Spatial_Weight_Type)) +
  geom_col(alpha = 0.7) +
  labs(title = "STARIMA Model Fit (uniform Weights - SLAG 1)",
...
if (nrow(param_table) > 0) {
  param_plot <- ggplot(param_table, aes(x = Parameter, y = Estimate, fill = Significant)) +
    geom_col(alpha = 0.7) +
    labs(title = "Parameter Estimates (uniform Model - SLAG 1)",
...
model_selection_results <- list(
  ...
  selected_model = list(
    ...
    object = uniform_results_slag1$model,
    results = uniform_results_slag1,
...
cat("\n=== SEASONAL MODEL SELECTION COMPLETED (uniform ONLY - SLAG 1) ===\n")
