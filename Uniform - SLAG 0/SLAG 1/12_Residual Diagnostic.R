...
cat("=== STARIMA RESIDUAL DIAGNOSTIC (uniform WEIGHTS - SLAG 1) ===\n\n")
...
cat(sprintf("🔬 Diagnostic Analysis for %s - uniform Weights (SLAG 1)\n\n", model_name))
...
model_fit <- uniform_results_slag1$model
...
# Create spatial weights list for residual analysis (SLAG 1 only)
wlist_residual <- list()
...
tryCatch({
  ...
  p_stacf_slag1 <- ggplot(stacf_slag1_data, aes(x = Lag, y = ACF)) +
    ...
    labs(title = "Residual STACF: uniform Weights - Spatial Lag 1",
         subtitle = "Neighbor residual autocorrelation (uniform matrix)",
         x = "Temporal Lag", y = "Residual STACF") +
    theme_minimal()
  ...
  p_stpacf_slag1 <- ggplot(stpacf_slag1_data, aes(x = Lag, y = PACF)) +
    ...
    labs(title = "Residual STPACF: uniform Weights - Spatial Lag 1",
         subtitle = "Neighbor residual partial autocorrelation (uniform matrix)",
         x = "Temporal Lag", y = "Residual STPACF") +
    theme_minimal()
