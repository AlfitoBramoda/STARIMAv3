...
# Purpose: Detailed residual analysis and visualization for uniform weights (SLAG 1)
# Author: STARMA Analysis - uniform Focus
# Date: 2024
# ============================================================================

cat("=== DETAILED RESIDUAL ANALYSIS (uniform WEIGHTS - SLAG 1) ===\n\n")
...
load("output/11_starima_uniform_slag1.RData")
...
P_order <- if (!is.null(uniform_results_slag1$orders$P)) uniform_results_slag1$orders$P else 0
Q_order <- if (!is.null(uniform_results_slag1$orders$Q)) uniform_results_slag1$orders$Q else 0
D_order <- if (!is.null(uniform_results_slag1$orders$D)) uniform_results_slag1$orders$D else 1
seasonal_period <- if (!is.null(uniform_results_slag1$orders$s)) uniform_results_slag1$orders$s else 12

model_name <- sprintf("STARIMA(%d,%d,%d) × (%d,%d,%d)%d", 
                     p_order, d_order, q_order, P_order, D_order, Q_order, seasonal_period)

cat(sprintf("📊 Detailed Residual Analysis for %s - uniform Weights (SLAG 1)\n\n", model_name))
...
# Extract residuals
residuals_matrix <- uniform_results_slag1$residuals
...
p1 <- ggplot(residual_long, aes(x = Time, y = Residual)) +
  ...
  labs(title = sprintf("Residual Time Series - %s (uniform Weights - SLAG 1)", model_name),
...
p2 <- ggplot(residual_long, aes(x = Residual)) +
  ...
  labs(title = sprintf("Residual Distribution - %s (uniform Weights - SLAG 1)", model_name),
...
p3 <- ggplot(residual_long, aes(sample = Residual)) +
  ...
  facet_wrap(~Region, scales = "free", ncol = 2) +
  labs(title = sprintf("Q-Q Plots - %s (uniform Weights - SLAG 1)", model_name),
...
p4 <- ggplot(residual_long, aes(x = Region, y = Residual, fill = Region)) +
  ...
  geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
  labs(title = sprintf("Residual Box Plots - %s (uniform Weights - SLAG 1)", model_name),
...
cat(sprintf("\n✅ Detailed residual analysis completed for %s - uniform Weights (SLAG 1)\n", model_name))
cat("📋 Final Model Summary:\n")
cat(sprintf("   - Model: %s\n", model_name))
cat(sprintf("   - Orders: (p,d,q,P,D,Q,s) = (%d,%d,%d,%d,%d,%d,%d)\n", 
           p_order, d_order, q_order, P_order, D_order, Q_order, seasonal_period))
cat(sprintf("   - Total Parameters: %d\n", uniform_results_slag1$fit_statistics$parameters))
