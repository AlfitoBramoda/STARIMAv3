...
if (exists("distance_results") && !is.null(distance_results$orders)) {
  p_order <- distance_results$orders$p %||% 0
  d_order <- distance_results$orders$d %||% 0
  q_order <- distance_results$orders$q %||% 0
  P_order <- distance_results$orders$P %||% 0
  D_order <- distance_results$orders$D %||% 0
  Q_order <- distance_results$orders$Q %||% 0
  seasonal_period <- distance_results$orders$s %||% 12
...
required_files <- c(
  ...
  "output/10_model_structure_distance_weights.RData"
)
...
if (file.exists("output/11_starima_distance.RData")) {
...
# ============================================================================
cat("\n🎯 SEASONAL STARIMA FORECASTING - distance WEIGHTS\n")
cat("=================================================\n")
...
for (t in 1:h) {
  for (region in 1:n_regions) {
    ...
    # 6. SPATIAL COMPONENT (Distance weights) - order-dependent strength
    spatial_component <- 0
    # Spatial strength varies by model complexity
    spatial_strength <- 0.15 * (1 + (p_order + q_order + P_order + Q_order) * 0.02)
...
cat("\n🔍 FINAL RMSE DEBUG:\n")
cat("===================\n")
for (i in 1:nrow(region_eval_distance)) {
  cat(sprintf("Region %s: RMSE = %.6f\n", region_eval_distance$Region[i], region_eval_distance$RMSE[i]))
}
cat("===================\n")
