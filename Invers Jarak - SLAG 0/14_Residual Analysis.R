# ============================================================================
# STARIMA Forecasting Pipeline - Phase 4: Residual Analysis (distance)
# File: 14_Residual_Analysis_distance.R
# Purpose: Detailed residual analysis and visualization for distance weights
# Author: STARMA Analysis - distance Focus
# Date: 2024
# ============================================================================

cat("=== DETAILED RESIDUAL ANALYSIS (distance WEIGHTS) ===\n\n")
...
P_order <- if (!is.null(distance_results$orders$P)) distance_results$orders$P else 0
Q_order <- if (!is.null(distance_results$orders$Q)) distance_results$orders$Q else 0
D_order <- if (!is.null(distance_results$orders$D)) distance_results$orders$D else 1
seasonal_period <- if (!is.null(distance_results$orders$s)) distance_results$orders$s else 12

model_name <- sprintf("STARIMA(%d,%d,%d) × (%d,%d,%d)%d", 
                     p_order, d_order, q_order, P_order, D_order, Q_order, seasonal_period)

cat(sprintf("📊 Detailed Residual Analysis for %s - distance Weights\n\n", model_name))
...
# Extract residuals
residuals_matrix <- distance_results$residuals
...
p1 <- ggplot(residual_long, aes(x = Time, y = Residual)) +
  ...
  labs(title = sprintf("Residual Time Series - %s (distance Weights)", model_name),
       subtitle = "Residuals should fluctuate randomly around zero",
       x = "Time", y = "Residual") +
  theme_minimal()

ggsave("plots/14_residual_timeseries_distance.png", p1, width = 12, height = 8, dpi = 300)
print(p1)
...
p2 <- ggplot(residual_long, aes(x = Residual)) +
  ...
  labs(title = sprintf("Residual Distribution - %s (distance Weights)", model_name),
       subtitle = "Residuals should be approximately normally distributed",
       x = "Residual", y = "Frequency") +
  theme_minimal()

ggsave("plots/14_residual_histogram_distance.png", p2, width = 12, height = 8, dpi = 300)
print(p2)
...
p3 <- ggplot(residual_long, aes(sample = Residual)) +
  ...
  facet_wrap(~Region, scales = "free", ncol = 2) +
  labs(title = sprintf("Q-Q Plots - %s (distance Weights)", model_name),
       subtitle = "Points should lie close to the red line for normality",
       x = "Theoretical Quantiles", y = "Sample Quantiles") +
  theme_minimal()

ggsave("plots/14_residual_qqplot_distance.png", p3, width = 12, height = 8, dpi = 300)
print(p3)
...
p4 <- ggplot(residual_long, aes(x = Region, y = Residual, fill = Region)) +
  ...
  geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
  labs(title = sprintf("Residual Box Plots - %s (distance Weights)", model_name),
...
ggsave("plots/14_residual_boxplot_distance.png", p4, width = 10, height = 6, dpi = 300)
print(p4)
...
if (assessment$model_adequate) {
  cat(sprintf("\n🎉 %s with distance weights is adequate for forecasting!\n", model_name))
  cat("✅ Model orders used: (p,d,q,P,D,Q,s) =", sprintf("(%d,%d,%d,%d,%d,%d,%d)\n", 
                                                        p_order, d_order, q_order, P_order, D_order, Q_order, seasonal_period))
} else {
  cat(sprintf("\n⚠️ %s with distance weights may need model refinement.\n", model_name))
  cat("⚠️ Model orders used: (p,d,q,P,D,Q,s) =", sprintf("(%d,%d,%d,%d,%d,%d,%d)\n", 
                                                        p_order, d_order, q_order, P_order, D_order, Q_order, seasonal_period))
}

cat(sprintf("\n✅ Detailed residual analysis completed for %s - distance Weights\n", model_name))
cat("📋 Final Model Summary:\n")
cat(sprintf("   - Model: %s\n", model_name))
cat(sprintf("   - Orders: (p,d,q,P,D,Q,s) = (%d,%d,%d,%d,%d,%d,%d)\n", 
           p_order, d_order, q_order, P_order, D_order, Q_order, seasonal_period))
cat(sprintf("   - Total Parameters: %d\n", distance_results$fit_statistics$parameters))
cat("📈 All visualizations saved to plots/ directory\n")
cat("🔗 distance-based spatial relationships analyzed\n")
