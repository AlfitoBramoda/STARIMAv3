...
# Load data
load("output/11_starima_distance_slag1.RData")   # distance_results_slag1
...
if (exists("distance_results_slag1") && !is.null(distance_results_slag1$model)) {
  model <- distance_results_slag1$model
...
if (exists("distance_results_slag1") && !is.null(distance_results_slag1$model)) {
  ...
  tryCatch({
    ...
    starma_model <- distance_results_slag1$model
        ...
        for (neighbor in 1:ncol(forecast_matrix)) {
          if (neighbor != col && col <= nrow(wlist[[2]]) && neighbor <= ncol(wlist[[2]])) {
            ...
            if (!is.na(weight) && weight > 0) {
              ...
              # Stronger spatial influence for distance weights
