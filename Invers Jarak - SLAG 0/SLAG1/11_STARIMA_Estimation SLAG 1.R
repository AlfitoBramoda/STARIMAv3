...
# Purpose: Estimate STARIMA(p,d,q) model using distance spatial weights (SLAG 1)
...
cat("=== STARIMA ESTIMATION - distance WEIGHTS (SLAG 1 FORCED) ===\n\n")
...
ok("Spatial weights list constructed with ", length(wlist_distance), " lags (SLAG 1)")
...
if (!inherits(fit, "white_noise_model") && nrow(coef_df) > 0) {
  ...
  cat("============================\n")
  cat(sprintf("Call: starma(data = data_input, wlist = wlist_distance, ar = ar_mask, ma = ma_mask)\n\n"))
...
cat("\n=== STARIMA ESTIMATION COMPLETED - distance WEIGHTS (SLAG 1 FORCED) ===\n")
