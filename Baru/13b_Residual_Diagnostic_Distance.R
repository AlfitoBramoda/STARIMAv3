...
# File: 11b_Residual_Diagnostic_distance.R
# Purpose: Test residual white noise properties for distance weights model
# Author: STARMA Analysis
# Date: 2024
# ============================================================================

# Load required data
load("output/10_model_structure.RData")    # Load model structure FIRST
load("output/11b_starima_distance.RData")
load("output/07_spatial_weights.RData")
load("output/05_differencing_results.RData")

# Debug: Check what we loaded
cat("📊 Loaded model parameters:\n")
cat("   - p_order:", p_order, "\n")
cat("   - d_order:", d_order, "\n")
cat("   - q_order:", q_order, "\n\n")

cat("=== RESIDUAL DIAGNOSTICS - distance WEIGHTS ===\n")
cat("Testing white noise properties of STARIMA(", p_order, ",", d_order, ",", q_order, ") residuals...\n\n")
...
# Extract residuals and parameters from loaded data
if (exists("starima_distance") && !is.null(starima_distance)) {
  residuals_distance <- as.vector(starima_distance$residuals)
  total_params <- length(starima_distance$phi) + length(starima_distance$theta)
} else if (exists("distance_results") && !is.null(distance_results$model)) {
  residuals_distance <- as.vector(distance_results$model$residuals)
  total_params <- length(distance_results$model$phi) + length(distance_results$model$theta)
} else {
  stop("❌ Cannot find STARIMA model or residuals in loaded data")
}

cat("- Residuals length:", length(residuals_distance), "\n")
cat("- Fitted parameters:", total_params, "\n")
cat("- Degrees of freedom:", length(residuals_distance) - total_params, "\n\n")
...
acf_significant <- sum(abs(residual_acf$acf[-1]) > 1.96/sqrt(length(residuals_distance)))
pacf_significant <- sum(abs(residual_pacf$acf) > 1.96/sqrt(length(residuals_distance)))
...
if (length(residuals_distance) <= 5000) {
  shapiro_test <- shapiro.test(as.vector(residuals_distance))
...
residuals_vec <- as.vector(residuals_distance)
...
acf_plot <- ggplot(acf_data, aes(x = Lag, y = ACF)) +
  ...
  labs(title = "Residual ACF - distance Weights",
...
pacf_plot <- ggplot(pacf_data, aes(x = Lag, y = PACF)) +
  ...
  labs(title = "Residual PACF - distance Weights",
...
qq_data <- data.frame(
  Theoretical = qnorm(ppoints(length(residuals_distance))),
  Sample = sort(as.vector(residuals_distance))
)

qq_plot <- ggplot(qq_data, aes(x = Theoretical, y = Sample)) +
  geom_point(alpha = 0.6, color = "darkblue") +
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +
  labs(title = "Q-Q Plot - distance Weights",
...
# Save plots
ggsave("plots/13b_distance_residual_acf.png", acf_plot, width = 10, height = 6, dpi = 300)
ggsave("plots/13b_distance_residual_pacf.png", pacf_plot, width = 10, height = 6, dpi = 300)
ggsave("plots/13b_distance_qq_plot.png", qq_plot, width = 8, height = 6, dpi = 300)

print(acf_plot)
print(pacf_plot)
print(qq_plot)

cat("✅ ACF plot saved: plots/113_distance_residual_acf.png\n")
cat("✅ PACF plot saved: plots/113_distance_residual_pacf.png\n")
cat("✅ Q-Q plot saved: plots/113_distance_qq_plot.png\n")
...
diagnostic_results <- list(
  model_type = paste0("STARIMA(", p_order, ",", d_order, ",", q_order, ") - distance Weights"),
  ...
  diagnostic_plots = c("11b_distance_residual_acf.png",
                       "11b_distance_residual_pacf.png",
                       "11b_distance_qq_plot.png")
)
...
save(diagnostic_results, residual_acf, residual_pacf, 
     file = "output/13b_diagnostic_distance.RData")
...
if(exists("stcor_result") && !is.null(stcor_result)) {
  tryCatch({
    ...
    ), title = "White Noise Test - distance")
  }, error = function(e) {
...
cat("\n=== RESIDUAL DIAGNOSTICS COMPLETED - distance WEIGHTS ===\n")
...
cat("✅ Results saved to: output/113_diagnostic_distance.RData\n")
...
cat("\n🎉 STARIMA(", p_order, ",", d_order, ",", q_order, ") distance weights diagnostic completed!\n")
