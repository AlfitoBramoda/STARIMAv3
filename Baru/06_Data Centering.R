...
if (exists("plot_dates")) {
  plot_dates <- as.Date(plot_dates)
} else {
  plot_dates <- seq(as.Date("2016-01-31"), by = "month", length.out = nrow(centered_matrix))
}
...
# Align training dates with train_data rows
if (length(dates) != nrow(train_data)) {
  diff_len <- length(dates) - nrow(train_data)
  if (diff_len > 0) {
    train_dates <- tail(as.Date(dates), nrow(train_data))
    cat("⚙️ Trimmed first", diff_len, "dates to align with train_data\n")
  } else {
    stop("❌ train_data has more rows than available dates. Check data split logic.")
  }
} else {
  train_dates <- as.Date(dates)
}

# Create synchronized data frame
...
print(p_centered)
ggsave("plots/06_centered_timeseries.png", p_centered, width = 12, height = 8, dpi = 300)
cat("✅ Centered time series plot saved: pplots/06_centered_timeseries.pngn")
...
ggsave("plots/06_centered_timeseries_free.png", p_centered_free, width = 12, height = 8, dpi = 300)
cat("✅ Centered time series (free Y) plot saved: plots/046centered_timeseries_free.png\n")
...
print(p_boxplot)
ggsave("plots/06_centered_boxplot.png", p_boxplot, width = 10, height = 6, dpi = 300)
cat("✅ Centered boxplot saved: plots/046centered_boxplot.png\n")
...
print(p_histogram)
ggsave("plots/06_centered_histogram.png", p_histogram, width = 12, height = 8, dpi = 300)
cat("✅ Centered histogram saved: plots/046centered_histogram.png\n")

# Save centered data and parameters
save(centered_matrix, centering_params, integration_order, 
     plot_dates, coordinates, original_stats, centered_stats,
