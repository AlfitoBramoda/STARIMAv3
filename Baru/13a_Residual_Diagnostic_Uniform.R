...
# File: 11a_Residual_Diagnostic_Uniform.R
# Purpose: Test residual white noise properties for uniform weights model
...
if (exists("starima_uniform") && !is.null(starima_uniform)) {
  residuals_uniform <- as.vector(starima_uniform$residuals)
  total_params <- length(starima_uniform$phi) + length(starima_uniform$theta)
} else if (exists("uniform_results") && !is.null(uniform_results$model)) {
  residuals_uniform <- as.vector(uniform_results$model$residuals)
  total_params <- length(uniform_results$model$phi) + length(uniform_results$model$theta)
} else {
  stop("❌ Cannot find STARIMA model or residuals in loaded data")
}

cat("- Residuals length:", length(residuals_uniform), "\n")
...
acf_significant <- sum(abs(residual_acf$acf[-1]) > 1.96/sqrt(length(residuals_uniform)))
pacf_significant <- sum(abs(residual_pacf$acf) > 1.96/sqrt(length(residuals_uniform)))
...
if (length(residuals_uniform) <= 5000) {
  shapiro_test <- shapiro.test(as.vector(residuals_uniform))
...
# Save plots
ggsave("plots/13a_uniform_residual_acf.png", acf_plot, width = 10, height = 6, dpi = 300)
ggsave("plots/13a_uniform_residual_pacf.png", pacf_plot, width = 10, height = 6, dpi = 300)
ggsave("plots/13a_uniform_qq_plot.png", qq_plot, width = 8, height = 6, dpi = 300)

print(acf_plot)
print(pacf_plot)
print(qq_plot)

cat("✅ ACF plot saved: plots/113_uniform_residual_acf.png\n")
cat("✅ PACF plot saved: plots/113_uniform_residual_pacf.png\n")
cat("✅ Q-Q plot saved: plots/113_uniform_qq_plot.png\n")
...
diagnostic_results <- list(
  ...
  diagnostic_plots = c("13a_uniform_residual_acf.png",
                       "13a_uniform_residual_pacf.png",
                       "13a_uniform_qq_plot.png")
)
...
save(diagnostic_results, residual_acf, residual_pacf, 
     file = "output/13a_diagnostic_uniform.RData")
...
cat("✅ Results saved to: output/113_diagnostic_uniform.RData\n")
...
cat("\n🎉 STARIMA(", p_order, ",", d_order, ",", q_order, ") uniform weights diagnostic completed!\n")
