...
if (exists("starima_correlation") && !is.null(starima_correlation)) {
  residuals_correlation <- as.vector(starima_correlation$residuals)
  total_params <- length(starima_correlation$phi) + length(starima_correlation$theta)
} else if (exists("correlation_results") && !is.null(correlation_results$model)) {
  residuals_correlation <- as.vector(correlation_results$model$residuals)
  total_params <- length(correlation_results$model$phi) + length(correlation_results$model$theta)
} else {
...
# Save plots
ggsave("plots/13c_correlation_residual_acf.png", acf_plot, width = 10, height = 6, dpi = 300)
ggsave("plots/13c_correlation_residual_pacf.png", pacf_plot, width = 10, height = 6, dpi = 300)
ggsave("plots/13c_correlation_qq_plot.png", qq_plot, width = 8, height = 6, dpi = 300)

print(acf_plot)
print(pacf_plot)
print(qq_plot)

cat("✅ ACF plot saved: plots/113_correlation_residual_acf.png\n")
cat("✅ PACF plot saved: plots/113_correlation_residual_pacf.png\n")
cat("✅ Q-Q plot saved: plots/113_correlation_qq_plot.png\n")
...
diagnostic_results <- list(
  model_type = paste0("STARIMA(", p_order, ",", d_order, ",", q_order, ") - correlation Weights"),
  ...
  diagnostic_plots = c("13c_correlation_residual_acf.png",
                       "13c_correlation_residual_pacf.png",
                       "13c_correlation_qq_plot.png")
)
...
save(diagnostic_results, residual_acf, residual_pacf, 
     file = "output/13c_diagnostic_correlation.RData")
...
cat("✅ Results saved to: output/113_diagnostic_correlation.RData\n")
...
cat("\n🎉 STARIMA(", p_order, ",", d_order, ",", q_order, ") correlation weights diagnostic completed!\n")
