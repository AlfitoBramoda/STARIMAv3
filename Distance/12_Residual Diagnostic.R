# ============================================================================
# RESIDUAL DIAGNOSTICS - DISTANCE WEIGHTS
# ============================================================================
...
wlist_distance <- list(diag(nrow(W)), W)

# Normalize weights
for (i in 1:length(wlist_distance)) {
  for (j in 1:nrow(wlist_distance[[i]])) {
    rs <- sum(wlist_distance[[i]][j, ])
    if (rs > 0) {
...
ggsave("plots/12_distance_residual_acf.png", acf_plot, width = 10, height = 6, dpi = 300)
ggsave("plots/12_distance_residual_pacf.png", pacf_plot, width = 10, height = 6, dpi = 300)
ggsave("plots/12_distance_qq_plot.png", qq_plot, width = 8, height = 6, dpi = 300)
...
wlist_residual[[2]] <- W              # Spatial lag 1 (Distance)
...
save(diagnostic_results, residual_acf, residual_pacf, 
     file = "output/12_diagnostic_distance.RData")

cat("\n✅ Diagnostic completed and saved to: output/12_diagnostic_distance.RData\n")
cat("✅ Plots saved to: plots/12_distance_*.png\n")