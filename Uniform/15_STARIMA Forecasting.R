...
# ----------------------------------------------------------------------------
# Load coefficients safely from uniform_results$model
# ----------------------------------------------------------------------------
if (exists("uniform_results") && !is.null(uniform_results$model)) {
  model <- uniform_results$model
  
  # Attempt to extract phi/theta safely
  if (!is.null(model$phi) && is.matrix(model$phi) && ncol(model$phi) >= 1) {
    phi <- as.matrix(model$phi)
    cat("Using ORIGINAL uniform phi coefficients:\n")
    print(head(phi, 3))
  } else if (!is.null(model$coefficients)) {
    # Sometimes stored as vector of coefficients
    phi <- matrix(model$coefficients[grep("phi", names(model$coefficients))], ncol = 1)
    cat("Extracted phi from coefficients vector\n")
  } else {
    phi <- matrix(c(0.4, 0.2, 0.1), ncol = 1)
    cat("Using default phi coefficients\n")
  }
  
  if (!is.null(model$theta) && is.matrix(model$theta) && ncol(model$theta) >= 1) {
    theta <- as.matrix(model$theta)
    cat("Using ORIGINAL uniform theta coefficients:\n")
    print(head(theta, 2))
  } else if (!is.null(model$coefficients)) {
    theta_vals <- model$coefficients[grep("theta", names(model$coefficients))]
    if (length(theta_vals) == 0) theta_vals <- c(0.3, 0.15)
    theta <- matrix(theta_vals, ncol = 1)
    cat("Extracted theta from coefficients vector\n")
  } else {
  ...
  phi <- matrix(c(0.4, 0.2, 0.1), ncol = 1)
  theta <- matrix(c(0.3, 0.15), ncol = 1)
  cat("Using default coefficients (uniform_results$model not found)\n")
}
