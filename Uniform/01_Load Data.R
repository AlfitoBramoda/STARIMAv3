...
# # 4. Seasonal Pattern (Monthly Average)
# plot_data_seasonal <- plot_data_long %>%
#   mutate(Month = format(Date, "%m"),
#          Year = format(Date, "%Y")) %>%
#   group_by(Month, Region) %>%
#   summarise(Avg_Rainfall = mean(Rainfall, na.rm = TRUE), .groups = "drop")

# p3 <- ggplot(plot_data_seasonal, aes(x = Month, y = Avg_Rainfall, color = Region, group = Region)) +
#   geom_line(size = 1) +
#   geom_point(size = 2) +
#   labs(title = "Seasonal Rainfall Pattern by Region",
#        x = "Month", y = "Average Rainfall (mm/day)",
#        subtitle = "Monthly averages across all years (2015-2024)") +
#   theme_minimal() +
#   theme(legend.position = "bottom")

# print(p3)
# ggsave("plots/01_seasonal_pattern.png", p3, width = 12, height = 6, dpi = 300)
# cat("✅ Seasonal pattern saved: plots/01_seasonal_pattern.png\n")
