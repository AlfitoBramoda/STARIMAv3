...
for (r in colnames(test_data)) {
  ...
  # Also save a combined plot showing both series clearly
  df_long <- tidyr::pivot_longer(df, cols = c("Actual", "Forecast"), 
                                names_to = "Type", values_to = "Value")
  
  p2 <- ggplot(df_long, aes(x = Time, y = Value, color = Type)) +
    geom_line(size = 1.2) +
    geom_point(size = 2) +
    scale_color_manual(values = c("Actual" = "black", "Forecast" = "red")) +
    labs(title = paste("Forecast vs Actual -", r, "(Enhanced)"),
         subtitle = "STARIMA Out-of-Sample (2024)",
         x = "Time", y = "Rainfall (standardized)") +
    theme_minimal() +
    theme(plot.title = element_text(hjust = .5),
  ...
  ggsave(paste0("plots/14_forecast_enhanced_", r, ".png"), p2, width = 10, height = 6, dpi = 300)
}
