...
tryCatch({
  load("output/10b_starima_distance.RData")  # Contains residual_acf, residual_pacf, etc.
  cat("✅ Distance estimation results loaded\n")
}, error = function(e) {
  cat("⚠️ Warning: Could not load 10b_starima_distance.RData. File may not exist yet.\n")
  cat("Please run 10b_STARIMA_Estimation_Distance.R first.\n")
})
...
if (exists("model_structures")) {
  p_order <- model_structures$distance$ar_order
  q_order <- model_structures$distance$ma_order
} else {
...
wlist_distance <- list(I, W)

# 3) Normalisasi baris + fallback self-weight utk baris nol
for (i in 2:length(wlist_distance)) {
  for (j in 1:nrow(wlist_distance[[i]])) {
    rs <- sum(wlist_distance[[i]][j, ], na.rm = TRUE)
    if (rs > 0) {
      wlist_distance[[i]][j, ] <- wlist_distance[[i]][j, ] / rs
    } else {
      # baris nol → beri self-weight
      wlist_distance[[i]][j, j] <- 1
    }
  }
}
cat("✅ Spatial weights normalized & isolated rows fixed (self-weight)\n")
# Opsional: verifikasi
print(sapply(wlist_distance, function(w) sum(w != 0)))

# 4) Tentukan total_params DARI MASK / HASIL ESTIMASI (paling akurat)
#    Jika pada estimasi Anda menyimpan AR/MA mask:
if (exists("model_structures")) {
  total_params <- model_structures$distance$total_params  # contoh: 18 (p=3,q=3, slag 0..2)
} else if (exists("distance_results") && !is.null(distance_results$fit_statistics$parameters)) {
  total_params <- distance_results$fit_statistics$parameters
...
cat("- Residuals dim:", paste(dim(residuals_distance), collapse="x"), "\n")
...
# Save plots
ggsave("plots/11a_distance_residual_acf.png", acf_plot, width = 10, height = 6, dpi = 300)
ggsave("plots/11a_distance_residual_pacf.png", pacf_plot, width = 10, height = 6, dpi = 300)
ggsave("plots/11a_distance_qq_plot.png", qq_plot, width = 8, height = 6, dpi = 300)

print(acf_plot)
print(pacf_plot)
print(qq_plot)

cat("✅ ACF plot saved: plots/11a_distance_residual_acf.png\n")
cat("✅ PACF plot saved: plots/11a_distance_residual_pacf.png\n")
cat("✅ Q-Q plot saved: plots/11a_distance_qq_plot.png\n")
...
diagnostic_results <- list(
  ...
  diagnostic_plots = c("11a_distance_residual_acf.png", 
                       "11a_distance_residual_pacf.png", 
                       "11a_distance_qq_plot.png")
)
...
save(diagnostic_results, residual_acf, residual_pacf, 
     file = "output/11b_diagnostic_distance.RData")
...
cat("✅ Results saved to: output/11a_diagnostic_distance.RData\n")
