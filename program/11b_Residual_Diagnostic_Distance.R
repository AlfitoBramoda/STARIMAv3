...
acf_plot <- ggplot(acf_data, aes(x = Lag, y = ACF)) +
  ...
  geom_hline(yintercept = c(-1.96/sqrt(length(residuals_distance)), 
                           1.96/sqrt(length(residuals_distance))), 
...
# Save plots
ggsave("plots/11b_distance_residual_acf.png", acf_plot, width = 10, height = 6, dpi = 300)
ggsave("plots/11b_distance_residual_pacf.png", pacf_plot, width = 10, height = 6, dpi = 300)
ggsave("plots/11b_distance_qq_plot.png", qq_plot, width = 8, height = 6, dpi = 300)

print(acf_plot)
print(pacf_plot)
print(qq_plot)

cat("✅ ACF plot saved: plots/11b_distance_residual_acf.png\n")
cat("✅ PACF plot saved: plots/11b_distance_residual_pacf.png\n")
cat("✅ Q-Q plot saved: plots/11b_distance_qq_plot.png\n")
...
cat("✅ Results saved to: output/11b_diagnostic_distance.RData\n")
cat("✅ Model adequacy:", ifelse(overall_adequate, "ADEQUATE", "NEEDS REVISION"), "\n\n")

cat("📊 PHASE 4 PROGRESS: 2/4 files completed (50%)\n")
cat("🎯 Next step: 11c_Residual_Diagnostic_Correlation.R\n\n")
