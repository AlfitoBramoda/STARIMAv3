...
# Visualization 3: Forecast Comparison for Best Region
best_region <- best_weights$Region[which.min(best_weights$Best_RMSE)]
cat("\n📈 Creating forecast comparison for best performing region:", best_region, "\n")

# Extract forecasts for the best region
forecast_comparison <- data.frame(
  Time = test_time,
  Actual = as.numeric(test_data[, best_region]),
  Uniform = as.numeric(results_uniform$forecast[, best_region]),
  Distance = as.numeric(results_distance$forecast[, best_region]),
  Correlation = as.numeric(results_correlation$forecast[, best_region])
)
...
p3 <- ggplot(forecast_long, aes(x = Time)) +
  ...
  labs(title = paste("Forecast Comparison -", best_region),
       subtitle = "All Weight Types vs Actual",
...
ggsave(paste0("plots/15_forecast_comparison_", best_region, ".png"), p3, 
