# ============================================================================
# STARMA Forecasting Pipeline - Phase 2: STPACF (correlation Only) - 95% CI Only
...
load("output/08_stacf_correlation_95only.RData")  # Use STACF results with 95% CI only

cat("=== STARMA STPACF ANALYSIS (correlation only) - 95% CI Only ===\n")
...
# Save plots
ggsave("plots/09_stpacf_correlation_slag0_95only.png", p_pacf_slag0, width = 10, height = 6, dpi = 300)
ggsave("plots/09_stpacf_correlation_slag1_95only.png", p_pacf_slag1, width = 10, height = 6, dpi = 300)

print(p_pacf_slag0)
print(p_pacf_slag1)

# --- AR order suggestion (95% only) ---
...
# Get AR order suggestions (95% CI)
correlation_ar <- suggest_ar_order_fixed(stpacf_correlation, conf_bound_95)

cat("\n🎯 AR Order Analysis (95% confidence):\n")
...
save(stpacf_correlation, correlation_ar, correlation_ma, d_order, stpacf_summary_fixed,
     conf_bound_95,
     file = "output/09_stpacf_correlation_95only.RData")

cat("\n✅ STPACF analysis completed (95% CI only)!\n")
cat("📊 Changes:\n")
