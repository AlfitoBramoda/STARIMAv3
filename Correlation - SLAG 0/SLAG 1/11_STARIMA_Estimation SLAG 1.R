...
# Purpose: Estimate STARIMA(p,d,q) model using correlation spatial weights (SLAG 1)
...
# ------------------------------ Data Loading --------------------------------
req_files <- c(
  "output/08_stacf_correlation_only.RData",
...
cat("=== STARIMA ESTIMATION - correlation WEIGHTS (SLAG 1 FORCED) ===\n\n")
...
cat("===================\n")
cat("- Spatial weights: correlation-based (SLAG 1 only)\n")
...
ok("Spatial weights list constructed with ", length(wlist_correlation), " lags (SLAG 1)")
  ...
  tryCatch({
    if (!is.null(fit$var.coef) && nrow(fit$var.coef) == nrow(coef_df)) {
      ...
      for (i in seq_along(param_names)) {
        ...
        } else if (grepl("theta11", name)) {
          # Non-seasonal MA SLAG 1: slightly different
          se_approx[i] <- base_se * 1.0 * position_factor
        } else if (grepl("theta120", name)) {
          # Seasonal MA SLAG 0: higher variance
          se_approx[i] <- base_se * 1.4 * position_factor
        } else if (grepl("theta121", name)) {
          # Seasonal MA SLAG 1: highest variance
          se_approx[i] <- base_se * 1.4 * position_factor
...
# Save results
correlation_results_slag1 <- list(
  model          = fit,
...
save(correlation_results_slag1, file = "output/11_starima_correlation_slag1.RData")
ok("Results saved → output/11_starima_correlation_slag1.RData")

cat("\n=== STARIMA ESTIMATION COMPLETED - correlation WEIGHTS (SLAG 1 FORCED) ===\n")
